...
| `vk` | Нет | ID записи в ВК. Пока что используется только для отображения ссылки под видео. |
| `hls` | Нет | Ссылка на HLS плейлист, содержащий данное видео. Желательно указывать вместе с `direct`, чтобы видео открылось даже там, где HLS не поддерживается. |
...
| `cuts` | стрим | Массив, содержащий диапазоны, вырезанные из **текущей** записи. Указанные здесь диапазоны будут вырезаны из таймкодов и субтитров с соответствующей коррекцией таймингов. Для субтитров создаётся отдельный файл `v{стрим}+{сегмент}.ass`, чтобы не терять оригинал. При удалении этого параметра побочные субтитры будут удалены автоматически. |
| `source_cuts` | стрим | Массив, содержащий диапазоны, вырезанные из **оригинальной** записи. Когда стрим временно прерывается, Twitch заменяет поток на статичную заставку. Эта заставка почему-то включается не сразу, из-за чего общая продолжительность стрима сокращается на случайную величину. Чат при этом не затрагивается, поэтому такие моменты нужно указывать для восстановления синхронизации субтитров и видео. В отличие от `cuts` не влияет на таймкоды в `timecodes.json` и `games.json`. |
...
Если при склейке меняется смещение фрагмента, нужно указать новое смещение в виде диапазона. В примере ниже при склейке терялось по 2 секунды в начале фрагментов:

```
"1877734451,1877816047,1877882907": {
  "offsets": ["0:00", "2:29:47+2", "3:44:15+2"]
}
```

Для проверки: конец диапазона должен точно указывать на видимое место склейки.
...
| `split` | Нет | Если установлено значение `year`, серия стримов будет автоматически поделена по годам. Год будет добавлен в ID и название прохождения. |
